function analyze_results(data_t,data_y,t_true,y_true,avar,mse,tau,experiment_name)
%% visualization
...
%         text(tau(idx_avar)*6/10,6/3*min_avar,['$\tau = ' num2str(round(tau(idx_avar),3)) '$ s'],'Interpreter','latex','FontSize',11)
...
%         text(round(tau(idx_mse),2)*6/10,6/3*min_mse,['$\tau = ' num2str(round(tau(idx_mse),3)) '$ s'],'Interpreter','latex','FontSize',11)
%         xlabel('Window length $\tau [s]$')
