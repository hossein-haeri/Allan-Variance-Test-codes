...
AVAR = [AVAR [num_window_samples,mean(EGNV)]'];
% figure(4)
% pv = plot_with_bounds(t,X(9,:),30, colors_p(w,:));
% pv.Color(4) = 0.8;
% pv = scatter(num_window_samples,mean(EGNV),'k');
% hold on
...
figure(4)
    xlabel('Number of records $M$','Interpreter','latex')
    ylabel('Mean eigenvalues deviation','Interpreter','latex')
%     colormap(colors_p);
%     Tk = linspace(1/(2*numel(window_length_list)),1 - 1/(2*numel(window_length_list)),numel(window_length_list));
%     cb = colorbar('Ticks', Tk, 'TickLabels',string(window_length_list));
%     cb.Label.String = 'window length [s]';
